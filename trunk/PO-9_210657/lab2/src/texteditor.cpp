    ...
    , ui(new Ui::TextEditor)
{
    ...
    //set saved in settings font to widget
    QSettings settings;
    auto font = settings.value("viewFont").value<QFont>();
    ui->textEdit->setCurrentFont(font);

    //connect close and exit buttons to predefined slots
    connect(ui->actionClose, SIGNAL(triggered()), this, SLOT(close()));
    connect(ui->actionExit, SIGNAL(triggered()), qApp, SLOT(closeAllWindows()));

    //connect text edit signal to documentModified slot
    connect(ui->textEdit, SIGNAL(textChanged()), this, SLOT(documentModified()));

    //connect text-changing functionality
...
void TextEditor::on_actionSelectFont_triggered()
{
    ...
    bool ok;
    auto newFont = QFontDialog::getFont(&ok, font);
    if (ok == true){
...
{
    QMessageBox::about(this, "О программе", "Посмотрите на наше волшебное, чудесное приложение. Съешьте за нас еще этих мягких французских булок, да выпейте чаю! ");
}
...
bool TextEditor::saveFile(){
    ...
    if (!file.open(QIODevice::WriteOnly  | QIODevice::Text)){
        QMessageBox::warning(this, "Warning", "Couldn't save the file");
        setFileName(QString());
        return false;
    }
