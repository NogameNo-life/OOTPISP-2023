...
void task1() {
    // Вариант 2
    // Значения переменных X, Y, Z (переменные вводятся в **lineedit**) поменять местами так, чтобы они оказались упорядоченными по убыван111ию.
    ...
    QObject::connect(button, &QPushButton::clicked, [=] {
        ...
        // Swap values to be in descending order
        if (x < y) std::swap(x, y);
        if (y < z) std::swap(y, z);
        if (x < y) std::swap(x, y);

        // Update lineEdit with swapped values
        lineEdit->setText(QString("%1 %2 %3").arg(x).arg(y).arg(z));
    });
